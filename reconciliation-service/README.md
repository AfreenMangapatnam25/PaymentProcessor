...
(`ddl-auto: validate`). Health is at `GET /actuator/health`.
...
```bash
# 1) create a run
curl -sX POST localhost:8080/api/v1/recon-runs -H 'Content-Type: application/json' -d '{
  "reconType":"GATEWAY","channel":"STRIPE","businessDate":"2026-07-18","currency":"USD","triggeredBy":"ops"
}'

# 2) add internal records
curl -sX POST localhost:8080/api/v1/recon-runs/1/records -H 'Content-Type: application/json' -d '{
  "records":[
...
curl -sX POST 'localhost:8080/api/v1/statements/ingest?runId=1' -H 'Content-Type: application/json' -d '{
...
curl -sX POST localhost:8080/api/v1/recon-runs/1/execute
curl -s 'localhost:8080/api/v1/recon-runs/1/exceptions'
```
