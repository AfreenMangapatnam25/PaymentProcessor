...
/**
 * Security wiring for the Ledger Service.
 *
 ...
 * header. This class turns the Ledger Service into an OAuth2 <b>resource server</b>: it fetches the
 * JWKS, verifies the signature, issuer, expiry and {@code purpose} claim locally, and only then
 * lets the request reach a controller that returns protected ledger data. Nothing calls back to the
 * authentication service on the hot path, which is what makes JWKS-based validation scale.
 *
 * <p><b>Why this service in particular.</b> The ledger is the immutable double-entry book of
 * record: journals, postings and reversals here are the financial truth the rest of the platform
 * reconciles against. Read access leaks the full money movement of every merchant, and write access
 * (especially reversals above the approval threshold) is the single most abuse-sensitive operation
 * on the platform, so no endpoint beyond probes and documentation is left anonymous.
 *
...
public class SecurityConfig {
     ...
     *       {@code @PreAuthorize("hasAuthority('SCOPE_ledger:read')")} on top of this coarse gate,
     *       so URL rules stay simple while sensitive operations (reversals, approvals) get precise
     *       checks.</li>
     * </ul>
     *
     ...
     * exists so the {@code local} profile &mdash; which loads seeded sample accounts and journals
     * through Flyway ({@code classpath:db/seed}) &mdash; can be exercised with plain {@code curl}
     ...
     *       {@code SCOPE_} prefix means {@code @PreAuthorize("hasAuthority('SCOPE_ledger:read')")}
