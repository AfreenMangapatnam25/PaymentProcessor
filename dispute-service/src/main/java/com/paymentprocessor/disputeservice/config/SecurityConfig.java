...
/**
 * Security wiring for the Dispute Service.
 *
 ...
 * header. This class turns the Dispute Service into an OAuth2 <b>resource server</b>: it fetches
 * the JWKS, verifies the signature, issuer, expiry and {@code purpose} claim locally, and only then
 * lets the request reach a controller that returns protected dispute data. Nothing calls back to
 * the authentication service on the hot path, which is what makes JWKS-based validation scale.
 *
 * <p><b>Why this service in particular needs it.</b> A dispute record ties a cardholder to a
 * merchant, an amount and a chargeback reason code, and the write endpoints move real money:
 * accepting liability, submitting representment evidence, escalating to arbitration. The
 * {@code ROLE_MERCHANT} vs {@code ROLE_ADMIN} distinction produced by
 * {@link #jwtAuthenticationConverter()} is what lets controllers keep a merchant inside their own
 * dispute set while an operations user works across all of them.
 *
 * <p><b>Outbound calls are unaffected.</b> The WebClient-backed integration clients in
 * {@code integration/web} call payment, ledger, settlement and notification services. This class
 * configures inbound protection only; it neither adds nor strips an {@code Authorization} header on
 * those outbound calls, so their existing behaviour is unchanged.
 *
 * <p><b>The single toggle.</b> {@code security.jwt.enabled} chooses between the enforcing chain in
 * {@link #jwtSecurityFilterChain(HttpSecurity, JwtAuthenticationConverter)} and the permit-all
 * chain in {@link #permitAllSecurityFilterChain(HttpSecurity)}. It defaults to <i>enabled</i>
 * ({@code matchIfMissing = true}) so that forgetting to set the property fails closed.
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
     ...
     *       {@code @PreAuthorize("hasAuthority('SCOPE_dispute:write')")} on top of this coarse
     *       gate, so URL rules stay simple while liability-changing operations get precise
     *       checks.</li>
     *   <li><b>Scheduled work is untouched</b> &mdash; {@code DeadlineMonitorJob} runs on a
     *       scheduler thread rather than an HTTP request, so it never passes through this chain and
     *       needs no token.</li>
     * </ul>
     *
     ...
     * exists so the {@code local} profile &mdash; which loads seeded sample disputes through
     ...
     *       {@code SCOPE_} prefix means {@code @PreAuthorize("hasAuthority('SCOPE_dispute:read')")}
