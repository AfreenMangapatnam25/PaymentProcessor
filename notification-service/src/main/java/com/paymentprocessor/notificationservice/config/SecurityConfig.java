...
/**
 * Security wiring for the Notification Service.
 *
 ...
 * token as {@code Authorization: Bearer ...}. This class turns the Notification Service
 * into an OAuth2 <b>resource server</b>: it fetches the JWKS once, caches it, and
 * verifies each token's signature, issuer, expiry and {@code purpose} claim locally
 * before any controller runs. No per-request call back to the auth service is made.
 *
 ...
 * the default and the only one that should ever run in a deployed environment; the
 * permit-all chain exists so the {@code local} profile — which loads seeded sample
 * templates, endpoints and messages through Flyway — can be exercised with plain
 * {@code curl} before anyone has started the authentication service.
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
    ...
    /**
     * Inbound webhook receivers for the outbound delivery providers.
     *
     * <p><b>These must remain unauthenticated, and that is a deliberate, uncomfortable
     * trade-off.</b> SendGrid and Twilio call these endpoints to report bounces,
     * spam complaints and delivery failures. They are third parties with no account on
     * this platform, so they cannot obtain or present one of our JWTs. Requiring a bearer
     * token here would simply break suppression-list maintenance.
     *
     * <p>The correct protection for these paths is <i>provider signature verification</i>
     * — SendGrid signs its event webhooks with Ed25519, Twilio with an
     * {@code X-Twilio-Signature} HMAC — which authenticates the caller without a shared
     * platform identity. {@code SuppressionController} explicitly notes that this
     * verification is <b>not yet implemented</b>. Until it is, these two endpoints accept
     * unsigned input from anyone who can reach them and should be restricted at the
     * network edge (allowlist the providers' published IP ranges at the gateway/ingress).
     */
    private static final String[] PROVIDER_WEBHOOK_PATHS = {
            "/api/suppressions/webhooks/sendgrid",
            "/api/suppressions/webhooks/twilio"
    };
     ...
     * {@link #PROVIDER_WEBHOOK_PATHS} needs a valid JWT.
     *
     ...
     *   <li><b>Webhook paths first</b> — matcher order matters. The provider webhook
     ...
     *       {@code @PreAuthorize("hasAuthority('SCOPE_notifications:write')")} on top of
     *       this coarse gate, keeping URL rules simple while sensitive operations get
     *       precise checks.</li>
     * </ul>
     *
    ...
    public SecurityFilterChain jwtSecurityFilterChain(
        ...
        log.warn("Provider webhook endpoints {} are exposed WITHOUT authentication; "
                + "provider signature verification is not yet implemented - restrict them "
                + "at the network edge.", String.join(", ", PROVIDER_WEBHOOK_PATHS));
        http
                .csrf(csrf -> csrf.disable())
                .sessionManagement(sm -> sm.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(PUBLIC_PATHS).permitAll()
                        // Third-party callbacks: see PROVIDER_WEBHOOK_PATHS javadoc.
                        .requestMatchers(PROVIDER_WEBHOOK_PATHS).permitAll()
     ...
     * It exists so the {@code local} profile — which loads seeded sample data through
     * Flyway — can be exercised with plain {@code curl} before the authentication service
     * is running. The toggle lives in configuration rather than code so the production
     * artifact stays byte-identical to the one developers run.
     *
     * @param http the chain builder supplied by Spring Security
     ...
     *       broad scope from ever being mistaken for administrator identity.</li>
     * </ul>
     *
     ...
     * refresh or step-up (MFA) ticket must never be accepted as an API credential, so
     * anything other than {@code purpose=access} is rejected here alongside the standard
     * expiry/not-before and issuer checks. The signature algorithm is pinned to RS256 so a
     * token cannot downgrade itself through its own header.
     *
