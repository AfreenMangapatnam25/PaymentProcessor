package com.paymentprocessor.fraudservice.config;
...
/**
 * Security wiring for the Fraud Service.
 *
 ...
 * header. This class turns the Fraud Service into an OAuth2 <b>resource server</b>: it fetches the
 * JWKS, verifies the signature, issuer, expiry and {@code purpose} claim locally, and only then
 * lets the request reach a controller that returns protected fraud data. Nothing calls back to the
 * authentication service on the hot path, which is what makes JWKS-based validation scale.
 *
 * <p><b>Why this service in particular needs it.</b> The fraud API exposes risk scores, rule
 * outcomes and the signals behind them. Those reveal how the detection engine reasons, so an
 * attacker who can read them can probe for the thresholds that let a fraudulent transaction pass.
 * Treating every endpoint as authenticated-by-default &mdash; {@code anyRequest().authenticated()}
 * rather than an allow-list of protected paths &mdash; means a newly added controller is closed
 * until someone deliberately opens it, instead of the other way round.
 *
...
public class SecurityConfig {
     ...
     *       {@code @PreAuthorize("hasAuthority('SCOPE_fraud:read')")} on top of this coarse gate,
     *       so URL rules stay simple while sensitive operations (overriding a rule outcome,
     *       manually clearing a flagged transaction) get precise checks.</li>
     * </ul>
     *
     ...
     * exists so the {@code local} profile &mdash; which loads seeded sample fraud rules and
     * transactions through Flyway &mdash; can be exercised with plain {@code curl} before anyone
     * has stood up the authentication service on port 8081. The toggle lives in configuration
     * rather than in code so the production artifact stays byte-identical to the one developers
     * run.
     *
     * @param http the chain builder supplied by Spring Security
     ...
     *       {@code SCOPE_} prefix means {@code @PreAuthorize("hasAuthority('SCOPE_fraud:read')")}
